...
// ROCKWOLF v37: 16-byte boot-failure trace appended after the 192-byte settings
// block. Lives at buffer offsets 192..207 (flash 0x7CC0..0x7CCF). Written only
// after a boot that sits un-armed for 3s, and wiped by the next settings save
// (page erase) — same lifetime as the v35 arming bytes 186/187. Read and
// decoded by RockwolfDiag.html.
#define RW_TRACE_OFF   192
#define RW_TRACE_SIZE  16
#define RW_TRACE_MAGIC 0x57 // 'W'
...
typedef union EEprom_u {
    ...
    uint8_t buffer[192 + RW_TRACE_SIZE];
} EEprom_t;
