...
/* v60: one extra slot for VREFINT. The F0 regular sequence converts in ASCENDING
   CHANNEL NUMBER order, not in the order the channels are added, so channel 17
   (VREFINT) always converts LAST: index 3 here, index 4 when the ADC-input build
   already uses 4 slots. */
#ifdef USE_ADC_INPUT
...
extern uint16_t ADC_raw_vrefint;   // v60: raw VREFINT count - VDDA in disguise
...
{ // read dma buffer and set extern variables
    ...
    ADC_raw_vrefint = ADCDataDMA[4];   // v60
    ...
    ADC_raw_vrefint = ADCDataDMA[3];   // v60
#endif
}
...
#ifdef USE_ADC_INPUT
    LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_1, 5);   // v60: + VREFINT
#else
    LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_1, 4);   // v60: + VREFINT
...
void ADC_Init(void)
{
    ...
    */
     ...
     */
    LL_ADC_REG_SetSequencerChAdd(ADC1, LL_ADC_CHANNEL_VREFINT);   // v60
    /** Configure Internal Channels: temp sensor AND the internal reference.
     *  VREFINT is a fixed voltage, so its raw count moves INVERSELY with VDDA -
     *  sampling it next to the pack reads the 3.3 V rail directly, in-run, at the
     *  full sample rate, with no meter and no extra pin. Sampling time is already
     *  71.5 cycles (~6 us at PCLK/4), above VREFINT's 4.6 us minimum. */
